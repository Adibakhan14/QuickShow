...
const AdminSidebar = () => {
  ...
  return (
    ...
    max-w-13 md:max-w-60 w-full border-r border-gray-300/20 text-sm'>
      <img className='h-9 md:h-14 w-9 md:w-14 rounded-full mx-auto' src={user.imgaeUrl}
      alt="sidebar" />
      ...
      <div className='w-full'>
        {adminNavLinks.map((link, index)=>(
            ...
            max-md:justify-center gap-2 w-full py-2.5 md:pl-10 first:mt-6 text-gray-400 ${isActive ? 'bg-primary/15 text-primary' : ''}`}>
              {({isActive}) => {
